...
#define MREQ_CURRENT_TASK   _IOW('a', 'a', int32_t *)
#define M_SIG_TX            44
...
static ssize_t m_write(struct file *file, const char __user *buf, size_t len,
                       loff_t *off);
static long m_ioctl(struct file *file, unsigned int cmd,
                    unsigned long arg);
...
static irqreturn_t m_irq_handler(int irq, void *dev_id)
{
    ...
    memset(&si, 0, sizeof(struct kernel_siginfo));
    si.si_signo = M_SIG_TX;
    si.si_code = SI_QUEUE;
    si.si_int = 1;

    if (task != NULL) {
        pr_info("Sending a signal to the app\n");
        if (send_sig_info(M_SIG_TX, &si, task) < 0) {
            pr_err("Unable to send a signal to the app\n");
        }
...
static long m_ioctl(struct file *file, unsigned int cmd,
                    unsigned long arg) {
    if (cmd == MREQ_CURRENT_TASK) {
        pr_info("Registering user space current task in driver\n");
        task = get_current();
        signo = M_SIG_TX;
    }
    return EXIT_SUCCESS;

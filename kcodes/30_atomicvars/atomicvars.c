...
static atomic_t mglobal_var = ATOMIC_INIT(0);
unsigned int mbit_check = 0;
...
static int m_thread_fn(void *data) {
    unsigned long mprev_val = 0;
    while (!kthread_should_stop()) {
        atomic_inc(&mglobal_var);
        mprev_val = test_and_change_bit(0, (void *)&mbit_check);
        pr_info("%s +%d: %s executing() mglobal_var: %u, mprev_val:%u\n",
                __FILE__, __LINE__, __func__, atomic_read(&mglobal_var), mprev_val);
        msleep(1000);
    }
    return EXIT_SUCCESS;
}
...
#if 1
    m_thread2 = kthread_run(m_thread_fn2, NULL, MY_KERNEL_THREAD);
    if (m_thread2) {
        pr_info("thread_create succeeded\n");
    } else {
        pr_info("thread_create failed\n");

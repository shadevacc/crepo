...
#define MY_KERNEL_THREAD1    "m_kernel_thread1"
#define MY_KERNEL_THREAD2    "m_kernel_thread2"

static dev_t dev_num = 0;
static volatile int m_val = 0;
static struct cdev m_cdev;
static struct class *m_class;
static struct task_struct *m_thread1;
static struct task_struct *m_thread2;
static seqlock_t m_seqlock;
static int m_global_var;
...
static ssize_t m_write(struct file *file, const char __user *buf, size_t len,
                       loff_t *off);
static int m_thread_fn1(void *data);
static int m_thread_fn2(void *data);
#if 0
...
static int m_thread_fn1(void *data) {
    int i = 0;
    pr_info("%s +%d: %s executing(): %d\n", __FILE__, __LINE__, __func__, i++);
    while (!kthread_should_stop()) {
        write_seqlock(&m_seqlock);
        m_global_var++;
        write_sequnlock(&m_seqlock);
        msleep(1000);
    }
    return EXIT_SUCCESS;
}
...
static int m_thread_fn2(void *data) {
    int i = 0;
    unsigned int m_seq_no = 0, m_read_val = 0;
    pr_info("%s +%d: %s executing(): %d\n", __FILE__, __LINE__, __func__, i++);
    while(!(kthread_should_stop())) {
        do {
            m_seq_no = read_seqbegin(&m_seqlock);
            m_read_val = m_global_var;
        } while(read_seqretry(&m_seqlock, m_seq_no));
        pr_info("m_seq_no = %d\nm_read_val = %d\nm_global_var = %d\n", m_seq_no,
                m_read_val, m_global_var);
        msleep(1000);
    }
...
static int __init m_init(void)
{
    ...
    seqlock_init(&m_seqlock);
    m_thread1 = kthread_create(m_thread_fn1, NULL, MY_KERNEL_THREAD1);
    if (m_thread1) {
        wake_up_process(m_thread1);
        pr_info("thread_create succeeded\n");
    } else {
        pr_info("thread_create failed\n");
        goto r_kthread_create1;
    }

#if 1
    m_thread2 = kthread_run(m_thread_fn2, NULL, MY_KERNEL_THREAD2);
    if (m_thread2) {
        pr_info("thread_create succeeded\n");
    } else {
        pr_info("thread_create failed\n");
        goto r_kthread_create2;
    }
#endif
#if 0

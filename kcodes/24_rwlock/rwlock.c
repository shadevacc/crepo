...
static int m_thread_1_fn(void *data) {
    ...
    while (!kthread_should_stop()) {
        write_lock(&m_rwlock);
        m_global_var++;
        write_unlock(&m_rwlock);
        pr_info("%s +%d: %s write: %d %d\n",
...
static int __init m_init(void)
{
    ...
    if (m_thread_2) {
        pr_info("m_thread_2 is allocated and triggered\n");
    } else {
        pr_info("m_thread_2 is not allocated and seeing problem\n");
        goto r_kthread2;
    }
#if 0
